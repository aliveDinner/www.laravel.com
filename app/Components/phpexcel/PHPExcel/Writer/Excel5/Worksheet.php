...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeSelection()
	{
		...
		list($colFirst, $rApp\Componentsirst) = PHPExcel_Cell::coordinateFromString($first);
		$colFirst = PHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index
		--$rApp\Componentsirst; // base 0 row index
		...
		$rApp\Componentsirst = min($rApp\Componentsirst, 65535);
		...
		$rwAct	= $rApp\Componentsirst;				// Active row
		$colAct   = $colFirst;			   // Active column
		$irefAct  = 0;					   // Active cell ref
		$cref	 = 1;					   // Number of refs

		if (!isset($rwLast)) {
			$rwLast   = $rApp\Componentsirst;	   // Last  row in reference
		}
		if (!isset($colLast)) {
		...
		if ($rApp\Componentsirst > $rwLast) {
			list($rApp\Componentsirst, $rwLast) = array($rwLast, $rApp\Componentsirst);
		}
