...
class Helper
{
     ...
     * @param bool $isSquare 为true 是正方形，false 则是定义的尺寸
     * @return bool
     *
     * list($width,$height,$type)=getimagesize($imgSrc);
     * 索引 0 给出的是图像宽度的像素值
     * 索引 1 给出的是图像高度的像素值
     * 索引 2 给出的是图像的类型，返回的是数字，其中1 = GIF，2 = JPG，3 = PNG，4 = SApp\Components，5 = PSD，6 = BMP，7 = TIFF(intel byte order)，8 = TIFF(motorola byte order)，9 = JPC，10 = JP2，11 = JPX，12 = JB2，13 = SWC，14 = IFF，15 = WBMP，16 = XBM
    ...
    public static function imageChangeByFormat($imgSrc, $imgDst = '', $format = '600*600')
    {
        ...
        $reHeight = 600;
        list($width, $height, $type) = getimagesize($imgSrc);
        $newWidth = ($width > $reWidth ? $reWidth : ($width > 6000 ? 6000 : $width));
...
//                $height = $height;
    ...
    public static function actionQrcodeOutByPaint($value = ' ', $logo = true)
    {
        ...
        //生成二维码图片
        $QR = \App\Components\QRcode::png($value, $out, $errorCorrectionLevel, $matrixPointSize, $margin, $saveandprint);
...
//                imagepng($QR, $out); //覆盖之前已经生成的原始二维码图
     ...
     * @param bool $step 为false按列读取，为true 按行读取
     * @return array
     */
    public static function importByPhpExcel($importFileName, $options = [], $sheetName = [], $importFileType = 'Excel2007', $step = true)
    {
        $ret = [];
        ...
        foreach ($objPHPExcel->getAllSheets() as $key => $value) {
            ...
            $result = [];
            $columns = [];
            $highestColumnLen = strlen($highestColumn);
            $highestColumnSum = 0;
            for ($i = 0; $i < $highestColumnLen; $i++) {
                $index = array_search($highestColumn[$i], $letter);
                $highestColumnSum += ($index + 1) * pow(26, $highestColumnLen - $i - 1);
            }
            $to = count($letter);
            $dict = implode('', $letter);
            for ($i = 0; $i <= $highestColumnSum; $i++) {
                $column = '';
                $num = $i;
                $level = 0;
                do {
                    $column = $dict[bcmod($num, $to) - $level] . $column; // 求模 //余数 //取进制值
                    $num = bcdiv($num, $to, 0); // 被除结果
                    if ($level == 0) {
                        $level = 1;
                    }
                } while ($num > 0);
...
//            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($curlPost)); //data with URLEncode
     ...
     * @return string
     */
    public static function getNumber($length = 4)
    {
        // 生成随机中文
        $zhSet = '0123456789';
        $code = '';
     ...
     * @param bool $onlyFirst 返回格式  是否只返回拼音首字母
     * @param string $delimiter 分隔符
     * @return string
     */
    public static function getZhPinYin($utf8Data, $onlyFirst = false, $delimiter = '')
    {
        return \App\Components\ChineseToPinyin::encode($utf8Data, $onlyFirst, $delimiter);
    }
}